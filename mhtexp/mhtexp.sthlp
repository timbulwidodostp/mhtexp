...
{phang}
{bf:mhtexp And mhtexp2} {hline 2} Stata command for the procedure detailed in List, Shaikh, and Vayalinkal and Xu (2015) And (2021) And (2023)
...
{phang}
By default {cmd:mhtexp And mhtexp2} will calculate all hypothesis based on the number of outcomes, subgroups and treatments provided by the user
in {it:varlist} {it:group(varname)} and {it:treatment(varname)}, respectively. In section 4.4 of List, Shaikh, and Vayalinkal and Xu (2015) And (2021) And (2023) simultaniously consider
...
{title:Examples}
{pstd}
Suppose a data set containing. You can access this dataset at https://raw.githubusercontent.com/timbulwidodostp/mhtexp/main/mhtexp/mhtexp.xlsx"{p_end}
...
{pstd}
Setup{p_end}
{phang} {cmd:. import excel "https://raw.githubusercontent.com/timbulwidodostp/mhtexp/main/mhtexp/mhtexp.xlsx", sheet("Sheet1") firstrow clear }{p_end}
...
{phang}{cmd:. mhtexp2 gave amount amountmat amountchange, treatment(treatment) }{p_end}
...
{phang}{cmd:. mhtexp2 gave, treatment(treatment) subgroup(groupid) }{p_end}

{pstd}
example 3: Hypothesis testing with multiple treatments{p_end}
{phang}{cmd:. mhtexp amount, treatment(ratio) }{p_end}
{phang}{cmd:. mhtexp2 amount, treatment(ratio) }{p_end}

{pstd}
Example 4: Hypothesis testing for all pairwise comparisons among the treatment and control groups{p_end}
{phang}{cmd:. mhtexp amount, treatment(ratio) combo("pairwise") }{p_end}
{phang}{cmd:. mhtexp2 amount, treatment(ratio) combo("pairwise") }{p_end}
